...
void forkTest(){
  int i;
  char * arr;
  arr = malloc (50000); //allocates 13 pages (sums to 16), OS puts page #15 in file.
...
Idea behind the algorithm:
	Space page will be swapped out sooner or later with fifo or lru.
	Since no one calls the space page, an extra page is needed to play with swapping (hence the #17).
	We selected a single page and reduced its page calls to see if fifo and lru will become more efficient.
...
void globalTest(){
	...
	int i;
	int randNum;
	arr = malloc(ARR_SIZE); //allocates 14 pages (sums to 17 - to allow more then one swapping in fifo)
